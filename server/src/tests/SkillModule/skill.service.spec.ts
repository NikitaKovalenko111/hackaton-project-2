...
describe('SkillService', () => {
  ...
  it('should throw error if company not found', async () => {
    ...
    await expect(service.createSkill('JS', 'Desc', 1)).rejects.toBeInstanceOf(
      ApiError,
    )
  })
  ...
  it('should give skill to employee', async () => {
    ...
    expect(mockSkillShapeRepo.findOne).toHaveBeenCalledWith({
      where: { skill_shape_id: 1 },
    })
  ...
  it('should throw error if skillShape not found', async () => {
    ...
    await expect(service.giveSkill(employee, 1, 3)).rejects.toBeInstanceOf(
      ApiError,
    )
  })
})
