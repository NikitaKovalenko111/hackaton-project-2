...
describe('InterviewService', () => {
  ...
  it('should add interview', async () => {
    ...
    (service['interviewRepository'].save as jest.Mock).mockResolvedValue(mockInterview);

    const result = await service.addInterview(mockSubject, new Date(), 'online', 'desc', 1);
  ...
  it('should throw error if employee has no company', async () => {
    ...
    await expect(service.addInterview({} as any, new Date(), 'online', 'desc', 1)).rejects.toThrow(ApiError);
  });

  it('should finish interview', async () => {
    const mockInterview = { interview_id: 1 };
    (service['interviewRepository'].findOne as jest.Mock).mockResolvedValue(mockInterview);
    (service['interviewRepository'].save as jest.Mock).mockResolvedValue({ ...mockInterview, interview_status: interviewStatusType.COMPLETED });
  ...
  it('should throw error if finishInterview not found', async () => {
    (service['interviewRepository'].findOne as jest.Mock).mockResolvedValue(null);
    await expect(service.finishInterview(1, 30, 'Good')).rejects.toThrow(ApiError);
  });

  it('should cancel interview', async () => {
    const mockInterview = { interview_id: 1 };
    (service['interviewRepository'].findOne as jest.Mock).mockResolvedValue(mockInterview);
    (service['interviewRepository'].save as jest.Mock).mockResolvedValue({ ...mockInterview, interview_status: interviewStatusType.CANCELED });
  ...
  it('should get planned interviews for HR/ADMIN/TEAMLEAD', async () => {
    ...
    (service['interviewRepository'].find as jest.Mock).mockResolvedValue(mockInterviews);
  ...
  it('should get planned interviews for regular employee', async () => {
    ...
    (service['interviewRepository'].find as jest.Mock).mockResolvedValue([{ interview_id: 2 }]);
    mockEmployeeService.getCleanEmployee.mockResolvedValue(mockEmployee);
