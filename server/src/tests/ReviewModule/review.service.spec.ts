...
const mockSocketGateway = {
  server: { emit: jest.fn(), to: jest.fn().mockReturnThis(), emit: jest.fn() },
}
...
describe('ReviewService', () => {
  ...
  it('should throw error if review not found on addQuestion', async () => {
    ...
    await expect(service.addQuestion('Test?', 1)).rejects.toBeInstanceOf(
      ApiError,
    )
  })
  ...
  it('should send answers', async () => {
    ...
    service['answerRepository'].save.mockResolvedValue([
      { answer_text: 'Answer' },
    ])
