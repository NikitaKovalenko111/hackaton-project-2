...
jest.mock('child_process', () => ({
  spawn: jest.fn(() => {
    return {
      stdout: {
        on: jest.fn((event, cb) => {
          if (event === 'data') cb(Buffer.from(JSON.stringify({ mocked: true })));
        }),
      },
      stderr: { on: jest.fn() },
    };
...
describe('StatisticsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        StatisticsService,
        { provide: getRepositoryToken(Statistics), useValue: mockStatisticsRepo },
  ...
  it('should generate skillsByCount statistics', async () => {
    mockSkillService.getSkillsByCompany.mockResolvedValue([{ skill_name: 'JS' }]);
