...
export class NotificationController {
  ...
  async getNotAppliedNotifications(
    ...
    try {
      ...
      const notifications =
        await this.notificationService.getNotAppliedNotifications(employeeId)
  ...
  async getAppliedNotifications(
    ...
    try {
      ...
      const notifications =
        await this.notificationService.getAppliedNotifications(employeeId)
  ...
  async getAllNotifications(@Req() request: Request): Promise<Notification[]> {
    try {
      ...
      const notifications =
        await this.notificationService.getAllNotifications(employeeId)
  ...
  async applyNotification(
    @Body() applyNotificationBody: applyNotificationBodyDto,
  ): Promise<Notification> {
    try {
      const { notification_id } = applyNotificationBody

      const notification =
        await this.notificationService.applyOneNotification(notification_id)
  ...
  @ApiOkResponse({
    type: [Notification],
    description: 'Список обновлённых уведомлений',
  })
  async applyAllNotification(@Req() req: Request): Promise<Notification[]> {
    try {
      ...
      const notifications =
        await this.notificationService.applyAllNotifications(employeeId)
