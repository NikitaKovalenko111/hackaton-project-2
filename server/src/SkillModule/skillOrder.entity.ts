...
export class SkillOrder {
  ...
  @ApiProperty({ type: () => SkillShape, description: 'Навык, к которому относится порядок развития' })
  ...
  @ApiProperty({ enum: skillLevel, example: skillLevel.JUNIOR, description: 'Уровень навыка' })
  @Column({
    type: 'enum',
    enum: skillLevel
  })
  ...
  @ApiProperty({ example: 'Изучить основы синтаксиса и типизацию', description: 'Описание задач для достижения уровня' })
  @Column({
    type: 'text'
  })
  order_text: string
